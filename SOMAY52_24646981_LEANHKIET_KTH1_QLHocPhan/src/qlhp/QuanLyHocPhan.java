...
public class QuanLyHocPhan {
   ...
   public boolean addHocPhan(HocPhan hocPhanMoi) {
    ...
    // cap nhat 
    public boolean capNhatDiemKetqua(String maSoHocVien ,  int diemKetQuaMoi)  {
    	for(HocPhan hp : dshp )  { 
    		 if(hp.getMssv().equalsIgnoreCase(maSoHocVien))  { 
    			  if (hp.getKetQua() == 0 ) { 
    				  hp.setKetQua(diemKetQuaMoi);
    				  System.out.println("cap nhat thanh cong trong mssv" ); 
        			  return true  ; 
    			  }else {  
    				  System.out.println(" loi sinh vien ");
    				  return false  ; 
    			  }
