...
public abstract class HocPhan {
    ...
    public void setHoTen(String hoTen) {
    	  if (hoTen == null || hoTen.trim().isEmpty()) {
              throw new IllegalArgumentException("ten sinh viên không được rỗng.");
          }
    ...
    public String toString() {
        DecimalFormat np = new DecimalFormat("###,###,###") ; 
       String hocPhiFormatted = np.format(hocPhi) ; 
       String tienThuongFormatted = np.format(tinhTienThuong()) ; 
        

        return String.format("%-7s | %-15s | %12s | %12s | %12s |    %10.0f | %10.0f | %12s |",
        		this.mssv,
                this.hoTen,
                hocPhiFormatted, 
                this.ngayBatDau.format(DateTimeFormatter.ofPattern("yyyy-MM-dd")),
                this.ngayKetThuc.format(DateTimeFormatter.ofPattern("yyyy-MM-dd")),
                this.diemMucTieu,
                this.ketQua,
                tienThuongFormatted 
